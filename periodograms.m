...
fMaxs1 = abs_Axis(abs_FFT_Index1)';
...
%     xlabel({['Freq. (Hz) ;'], ['f_{max} = ',num2str(fMaxs1),' Hz']},'FontSize',8);
...
%     xlabel({['Freq. (Hz) ;'], ['f_{max} = ',num2str(fMaxs2),' Hz']},'FontSize',8);
%     ylabel('Power/Frequency' ,'FontSize',8);
% 
%     
% end
...
%     xlabel({['Freq. (Hz) ;'], ['f_{max} = ',num2str(fMaxs3),' Hz']},'FontSize',8);
%      ylabel('Power/Frequency' ,'FontSize',8);
...
fMaxs = [transpose(fMaxs1) fMaxs2 fMaxs3]
abs_FFT_Value = [transpose(abs_FFT_Value1) transpose(abs_FFT_Value2) transpose(abs_FFT_Value3)]
